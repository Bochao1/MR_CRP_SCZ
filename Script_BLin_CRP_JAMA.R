...
##      author: Bochao Lin (validated by Yue Li)
##        date: 7th Dec 2017
##-------------------------------------
...
mr_egger_random_MR2_intercept=round(exp(c(x@Intercept,x@CILower.Int,x@CIUpper.Int)),2)
...
resMR2=data.frame(package=rep('MendelianRandomization',5),
                  model=c("ivw_fixed","ivw_random","egger_random","egger_random_intercept",'weighted_mdeilan'),
                  rbind(mr_ivm_fixed_MR2,mr_ivm_random_MR2,mr_egger_random_MR2,mr_egger_random_MR2_intercept,mr_median_MR2))
...
##Compare estimates from three different methods (OR estimates with Ln increment):
com.res=rbind(res_TwosampleMR,resMR2,resManual) 
#these are estimates with increment of Ln 
##Get OR estimates with 2-folder increments.
e=exp(1)

com.res$OR_2fold=round(((com.res$OR)^(1/e))^2,2)
com.res$SE.low_2fold=round(((com.res$SE.low)^(1/e))^2,2)
com.res$SE.up_2fold=round(((com.res$SE.up)^(1/e))^2,2)
colnames(com.res)=c('package','model','OR_lnfold','SE.low_lnfold','SE.up_lnfold','OR_2fold','SE.low_2fold','SE.up_2fold')
View(com.res)
...
mr_egger_random_MR2_OR=round(exp(c(x@Estimate,x@CILower.Est,x@CIUpper.Est)),2)
mr_egger_random_MR2_INT=round(exp(c(x@Intercept,x@CILower.Int,x@CIUpper.Int)),2)
x=MendelianRandomization::mr_median(MRInputObject2,weighting = "weighted", distribution = "normal")
mr_median_MR2=round(exp(c(x@Estimate,x@CILower,x@CIUpper)),2)
resHartwig=data.frame(model=c("ivw_fixed","ivw_random","egger_random","egger_random",'weighted_mdeilan'),
                  rbind(mr_ivm_fixed_MR2,mr_ivm_random_MR2,mr_egger_random_MR2_OR,mr_egger_random_MR2_INT,mr_median_MR2))
colnames(resHartwig)=c('model','OR_lnfold','SE.low_lnfold','SE.up_lnfold')
resHartwig$OR_2fold=round(((resHartwig$OR_lnfold)^(1/e))^2,2)
resHartwig$SE.low_2fold=round(((resHartwig$SE.low_lnfold)^(1/e))^2,2)
resHartwig$SE.up_2fold=round(((resHartwig$SE.up_lnfold)^(1/e))^2,2)
View(resHartwig)

# Even when using the same input data as Hartwig et al, some of the estimates we get are different from Hartwig table 1.
